...
void header() {//prints header
  cout << line << '\n' << sep//declaring fields to go in header columns in order from left to right
...
void clearScreen() { //clears the screen to make output neater.
...
int main() {
  ...
  const int name_width = 15 ;//max# of spaces for string fields.
  const int int_width = 7 ;//max# of spaces for interger fields.
  const int dbl_width = 12 ;//max# of spaces for double fields.
  const int num_flds = 7 ;//max# of culumns in list.
  const std::string sep = " |" ;
  const int total_width = name_width*2 + int_width*2 + dbl_width*3 + sep.size() * num_flds ;//length of list to accomodate the list parameters
  const std::string line = sep + std::string( total_width-1, '-' ) + '|' ;// list border character choice, ex. '-' or '|'

  vector<Country> countryList(228);//declare vector for 227 countries
